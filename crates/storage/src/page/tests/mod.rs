use crate::page::layout::header::HeaderRef;
use crate::page::layout::page::Page;
use crate::page::layout::slot::SLOT_SIZE;
...
impl Page {
    ...
    pub fn test_create_empty_heap() -> Page {
    ...
    pub fn test_set_slot_count(&mut self, count: usize) {
        ...
        let result = header_mut.set_free_end(new_free_end as u16);
        assert!(result.is_ok());
    }

    #[expect(unused)]
    /// Populates the slot array with the provided slots. Sets up the header with the correct slot count as well.
    pub(crate) fn test_create_slots(&mut self, slots: Vec<SlotValues>) {
    ...
    /// ```
    pub(crate) fn test_insert_rows(&mut self, slots: Vec<SlotValues>) {
        self.test_set_slot_count(slots.len());
        for (index, slot) in slots.iter().enumerate() {
            let mut slot_array_mut = self.slot_array_mut().unwrap();
            slot_array_mut
    ...
    pub(crate) fn assert_row_values(&mut self, offset: usize, length: usize, value: u8) {
        let actual_row_data = &mut self.data_mut()[offset..offset + length];

        assert_eq!(vec![value; length].as_slice(), actual_row_data);
    }

    pub(crate) fn assert_slot(&mut self, slot_index: usize, offset: usize, length: usize) {
        let slot = self
            .slot_array_ref()
            .unwrap()
    ...
    /// )
    /// ```
    pub fn assert_header(&mut self, assertions: &[&dyn Fn(&HeaderRef)]) {
    ...
    macro_rules! impl_expect_ref {
        ($enum:ty, $fn_name:ident, $variant:ident => $inner:ty) => {
            impl $enum {
                #[allow(unused)]
                #[track_caller]
                pub(crate) fn $fn_name(&self) -> &$inner {
                    match self {
